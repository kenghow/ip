public class Parser {
    ...
    public Task parseTask(String input) {
        ...
        throw new IllegalArgumentException("I don't understand this command.");
    }

    private Deadline parseDeadline(String rest) {
        String[] parts = rest.split(" /by", 2);
        if (parts.length < 2) {
            throw new IllegalArgumentException("Deadline format: deadline <desc> /by <time>");
        }
    ...
    private Event parseEvent(String rest) {
    ...
    private void ensureNotEmpty(String s, String message) {
        if (s.isEmpty()) {
